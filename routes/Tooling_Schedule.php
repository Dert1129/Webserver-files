...
function Tooling(){
    ...
    $stmt = $conn->prepare("SELECT * FROM Tooling_Schedule;");
    $stmt->execute();
    ...
    if ($result !==false){
        while ($row = $result->fetch_assoc()){
            ...
            if(strlen($row["Part_Number"])==2){
                ...
                echo "<td class='col-2 $text' style='height:8rem'>".$row['Thumbnail']."</td>";
            }elseif($row['Thumbnail']=="Tooling.png"){
                echo "<td class='col-2'>". "<img class='lozad' id='Thumbnail' data-src='../Thumbnails/Stock Images/".$row['Thumbnail']."' width='170px' height='112px'>". "</td>";
            ...
            echo "<td class='col-1 $text' style='height:8rem'>".$row['Qty_To_Stock']."</td>";  
            echo "<td class='col-1 $text' style='height:8rem'>".$row['Design_Complete']."</td>";
